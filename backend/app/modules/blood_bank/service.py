...
class BloodBankService:
    ...
    def dispatch_unit(self, unit_id: int, data: UnitDispatch) -> UnitOut:
        ...
        if not unit.is_safe:
            raise HTTPException(
                ...
                detail="Unit has not passed quality check — cannot dispatch.",
            )
    ...
    def broadcast_shortage(self, data: ShortageAlertIn, blood_bank_user_id: int) -> dict:
        ...
        self.notif.broadcast_to_donors(
            blood_group=data.blood_group,
            title=f"🩸 Blood Shortage — {data.blood_group.value}",
            body=msg,
        )
        ...
        try:
            ...
            # No running event loop (e.g., in tests) — skip pub/sub silently
            pass
    ...
    # ── Blood Bank Profile CRUD & Nearest ─────────────────────────────────────
